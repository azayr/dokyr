<script>
  ...
  let setupDNS = { hostname: '', ipv4Addresses: [], ipv6Addresses: [], verified: false, status: 'idle', message: '' };
  let smtpPort = 465;
  ...
  $: setupDNSVerified = setupDNS.verified && setupDNS.hostname === setupMailHostname;
  $: setupRecordValue = setupDNS.hostname === setupMailHostname && setupDNS.ipv4Addresses?.length ? setupDNS.ipv4Addresses[0] : '';
  ...
  async function setupMailServer() {
    if (!setupDNSVerified) {
      error = 'Verify the required A record before completing mail server setup.';
      return;
    }
    saving = true;
    error = '';
  ...
  async function verifyMailSetupDNS() {
    setupChecking = true;
    error = '';
    setupDNS = { hostname: setupMailHostname, ipv4Addresses: [], ipv6Addresses: [], verified: false, status: 'checking', message: 'Checking public DNS…' };
    try {
      const response = await api('/api/mail/setup/verify-dns', {
        method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ domain: setupDomain })
      });
      const payload = await readAPIJSON(response);
      if (!response.ok) throw new Error(payload.error || 'Could not verify the mail hostname');
      setupDNS = payload;
      if (payload.verified) toast.success(`${payload.hostname} is visible in public DNS.`);
    } catch (cause) {
      const message = cause instanceof Error ? cause.message : 'Could not verify DNS';
      setupDNS = { hostname: setupMailHostname, ipv4Addresses: [], ipv6Addresses: [], verified: false, status: 'error', message };
      error = message;
    } finally {
      setupChecking = false;
    }
    ...
    <section class="panel setup-panel">
      ...
      {#if canSetupMail}
        <form onsubmit={(event) => { event.preventDefault(); setupMailServer(); }}>
          <div class="setup-domain-entry">
            <label class="field"><span>Dokyr domain</span><input class="input input-mono" bind:value={setupDomain} placeholder="example.com" autocomplete="off" spellcheck="false" required /><small>Dokyr reserves <code>{setupMailHostname}</code> as the SMTP and TLS hostname.</small></label>
            <button class="btn" type="button" disabled={setupChecking || !setupDomain.trim()} onclick={verifyMailSetupDNS}><Icon name="refresh" size={13} />{setupChecking ? 'Checking…' : 'Verify DNS'}</button>
          </div>

          <div class="setup-dns-card">
            <header>
              <div><span class="eyebrow">Required DNS record</span><strong>Point mail traffic to this VPS</strong></div>
              <span class="setup-dns-state {setupDNSVerified ? 'verified' : setupDNS.status === 'incorrect' || setupDNS.status === 'error' ? 'incorrect' : ''}"><Icon name={setupDNSVerified ? 'check-circle' : setupDNS.status === 'incorrect' || setupDNS.status === 'error' ? 'alert' : 'clock'} size={12} />{setupDNSVerified ? 'Verified' : setupDNS.status === 'checking' ? 'Checking' : setupDNS.status === 'incorrect' || setupDNS.status === 'error' ? 'Needs attention' : 'Not verified'}</span>
            </header>
            <div class="setup-record-labels" aria-hidden="true"><span>Status</span><span>Type</span><span>Name</span><span>Value</span></div>
            <div class="setup-record-row">
              <span class="record-status" class:verified={setupDNSVerified} class:incorrect={setupDNS.status === 'incorrect' || setupDNS.status === 'error'}><Icon name={setupDNSVerified ? 'check' : setupDNS.status === 'incorrect' || setupDNS.status === 'error' ? 'alert' : 'clock'} size={12} /></span>
              <span class="record-type">A</span>
              <span class="setup-copy-cell"><code>{setupMailHostname}</code><button type="button" aria-label="Copy DNS record name" title="Copy name" onclick={() => copy(setupMailHostname, 'DNS record name')}><Icon name="copy" size={13} /></button></span>
              <span class="setup-copy-cell"><code class:placeholder={!setupRecordValue}>{setupRecordValue || 'Your VPS public IPv4'}</code><button type="button" aria-label="Copy DNS record value" title={setupRecordValue ? 'Copy value' : 'Verify DNS to detect the value'} disabled={!setupRecordValue} onclick={() => copy(setupRecordValue, 'DNS record value')}><Icon name="copy" size={13} /></button></span>
            </div>
            <footer class:verified={setupDNSVerified}><Icon name={setupDNSVerified ? 'check-circle' : 'info'} size={14} /><span>{setupDNS.hostname === setupMailHostname && setupDNS.message ? setupDNS.message : 'Create this record as DNS only, wait for propagation, then verify it here. Port 80 must remain reachable for TLS.'}</span></footer>
          </div>

          <div class="setup-actions"><span>{setupDNSVerified ? 'DNS is ready. You can complete the one-time setup.' : 'Complete setup unlocks after DNS verification.'}</span><button class="btn btn-primary" type="submit" disabled={saving || !setupDNSVerified}>{saving ? 'Setting up…' : `Complete setup`}</button></div>
        </form>
      {:else}
    ...
    {:else}
      <div class="domain-workspace">
        ...
        {#if selected}
          ...
          {@const requiredRecords = (selected.records || []).filter((record) => record.required)}
          {@const optionalRecords = (selected.records || []).filter((record) => !record.required)}
          <section class="domain-detail panel">
            ...
            <div class="records-heading">
              <div><strong>Required for sending</strong><small>Add these records before verifying your domain.</small></div>
              <span>{requiredRecords.length} record{requiredRecords.length === 1 ? '' : 's'}</span>
            </div>
            <div class="records-table" role="table" aria-label="Required DNS records">
              <div class="record-row record-header" role="row"><span>Status</span><span>Type</span><span>Name</span><span>Value</span></div>
              {#each requiredRecords as record}
                <div class="record-row" role="row">
                  ...
                  <span class="record-value"><small>{record.purpose}</small><span class="copyable"><code title={record.name}>{record.name}</code><button type="button" aria-label={`Copy ${record.purpose} name`} title="Copy name" onclick={() => copy(record.name, `${record.purpose} name`)}><Icon name="copy" size={13} /></button></span></span>
                  <span class="record-value"><span class="copyable"><code title={record.value}>{record.value}</code><button type="button" aria-label={`Copy ${record.purpose} value`} title="Copy value" onclick={() => copy(record.value, `${record.purpose} value`)}><Icon name="copy" size={13} /></button></span></span>
                </div>
              {/each}
            </div>

            {#if optionalRecords.length > 0}
              <details class="optional-records">
                <summary><span><strong>Optional DNS records</strong><small>Recommended security and mail-client discovery records. They do not block sending.</small></span><span class="optional-count">{optionalRecords.length}<Icon name="chevron-down" size={14} /></span></summary>
                <div class="records-table" role="table" aria-label="Optional DNS records">
                  <div class="record-row record-header" role="row"><span>Status</span><span>Type</span><span>Name</span><span>Value</span></div>
                  {#each optionalRecords as record}
                    <div class="record-row" role="row">
                      <span class="record-status {record.status}" title={record.lastError || record.status}><Icon name={record.status === 'verified' ? 'check' : record.status === 'incorrect' ? 'alert' : 'clock'} size={12} /></span>
                      <span><code class="record-type">{record.type}</code>{#if record.priority}<small>Priority {record.priority}</small>{/if}</span>
                      <span class="record-value"><small>{record.purpose}</small><span class="copyable"><code title={record.name}>{record.name}</code><button type="button" aria-label={`Copy ${record.purpose} name`} title="Copy name" onclick={() => copy(record.name, `${record.purpose} name`)}><Icon name="copy" size={13} /></button></span></span>
...
<style>
  ...
  .setup-panel { padding: var(--space-6); display: grid; gap: var(--space-6); }.setup-copy { display: flex; align-items: flex-start; gap: var(--space-4); }.setup-icon { width: 46px; height: 46px; flex: 0 0 auto; display: grid; place-items: center; border-radius: var(--radius-md); background: var(--color-accent-softer); color: var(--color-accent); }.setup-copy h2 { margin: 5px 0 5px; font-size: var(--text-lg); }.setup-copy p { max-width: 720px; margin: 0; color: var(--color-muted); font-size: var(--text-sm); line-height: 1.55; }.setup-panel form { display: grid; gap: var(--space-4); }.setup-domain-entry { display: grid; grid-template-columns: minmax(0, 1fr) auto; align-items: end; gap: var(--space-3); }.setup-domain-entry .btn { margin-bottom: 1px; }.setup-dns-card { overflow: hidden; border: 1px solid var(--color-rule); border-radius: var(--radius-lg); background: var(--color-paper-raised); }.setup-dns-card > header { min-height: 66px; padding: 12px var(--space-4); display: flex; align-items: center; justify-content: space-between; gap: var(--space-3); border-bottom: 1px solid var(--color-rule); background: linear-gradient(110deg, var(--color-paper-subtle), var(--color-paper-raised)); }.setup-dns-card > header > div { display: grid; gap: 3px; }.setup-dns-card > header strong { font-size: var(--text-sm); }.setup-dns-state { padding: 4px 8px; display: inline-flex; align-items: center; gap: 5px; border-radius: 99px; background: var(--color-warning-soft); color: var(--color-warning); font-size: var(--text-2xs); font-weight: 700; }.setup-dns-state.verified { background: var(--color-success-soft); color: var(--color-success); }.setup-dns-state.incorrect { background: var(--color-danger-soft); color: var(--color-danger); }.setup-record-labels, .setup-record-row { display: grid; grid-template-columns: 52px 58px minmax(190px, .9fr) minmax(220px, 1.1fr); align-items: center; gap: 8px; }.setup-record-labels { min-height: 30px; padding: 0 var(--space-4); border-bottom: 1px solid var(--color-rule); background: var(--color-paper-subtle); color: var(--color-muted); font-size: 9px; font-weight: 700; letter-spacing: .04em; text-transform: uppercase; }.setup-record-row { min-height: 64px; padding: 8px var(--space-4); }.setup-copy-cell { min-width: 0; display: flex; align-items: center; gap: 5px; }.setup-copy-cell code { min-width: 0; flex: 1; overflow: hidden; color: var(--color-ink-secondary); font-size: var(--text-xs); text-overflow: ellipsis; white-space: nowrap; }.setup-copy-cell code.placeholder { color: var(--color-muted); font-family: var(--font-sans); font-size: var(--text-2xs); }.setup-copy-cell button { width: 29px; height: 29px; flex: 0 0 auto; display: grid; place-items: center; border: 0; border-radius: var(--radius-xs); background: transparent; color: var(--color-muted); cursor: pointer; }.setup-copy-cell button:hover:not(:disabled) { background: var(--color-paper-subtle); color: var(--color-accent); }.setup-copy-cell button:disabled { opacity: .35; cursor: not-allowed; }.setup-dns-card > footer { min-height: 40px; padding: 8px var(--space-4); display: flex; align-items: center; gap: 7px; border-top: 1px solid var(--color-rule); background: var(--color-warning-soft); color: var(--color-warning); font-size: var(--text-2xs); }.setup-dns-card > footer.verified { background: var(--color-success-soft); color: var(--color-success); }.setup-actions { display: flex; align-items: center; justify-content: flex-end; gap: var(--space-4); }.setup-actions > span { color: var(--color-muted); font-size: var(--text-2xs); }.setup-actions .btn { min-width: 132px; }
